...
Rem
Rem    DESCRIPTION
Rem      BAK_RPT_TIMETABLES_SPECS
Rem
Rem    MODIFIED   (YYMMDD)
Rem    appler      160516 - Created.

prompt ... creating view "BAK_RPT_TIMETABLES_SPECS"
create or replace view BAK_RPT_TIMETABLES_SPECS as
select
  ts.id                as id
 ...
 ,p.name               as post_name
 ,to_char(ts.wage_rate,'0D0')  as wage_rate
...
-- ,pkg_rpt_timetables.timetable_spec_hazard_prev (t.table_year,t.table_month,ts.emp_id,ts.post_id, ts.wage_rate) hours_hazard_prev
 ...
 ,pkg_rpt_timetables.timetable_spec_holidays(ts.id)      as holidays_total
 ,pkg_rpt_timetables.timetable_spec_dayoffs(ts.id)       as holidays_dayoffs
