...
Rem
Rem    DESCRIPTION
Rem      RPT_TIMETABLES_SPECS
Rem
Rem    MODIFIED   (YYMMDD)
Rem    appler      160516 - Created.

prompt ... creating view "RPT_TIMETABLES_SPECS"
create or replace view RPT_TIMETABLES_SPECS as
select
  ts.id            as id
 ...
 ,p.name           as post_name
 ,trim(to_char(ts.wage_rate,'0D0'))                     as wage_rate
 ...
 ,pkg_rpt_timetables.timetable_spec_days_totals(ts.id)  as days_total
 ,pkg_rpt_timetables.days_15_totals(ts.id)              as days_15_total
 ,pkg_rpt_timetables.timetable_spec_hours_totals(ts.id) as hours_total
 ,pkg_rpt_timetables.hours_15_totals(ts.id)             as hours_15_total
 ,pkg_rpt_timetables.timetable_spec_hours_night(ts.id)  as hours_night
 ,pkg_rpt_timetables.timetable_spec_hazard_prev(ts.id)  as hours_hazard_prev
 ,pkg_rpt_timetables.timetable_spec_hazard(ts.id)       as hours_hazard_cur
 ,pkg_rpt_timetables.timetable_spec_holidays(ts.id)     as hours_holidays
 ,pkg_rpt_timetables.timetable_spec_dayoffs(ts.id)      as hours_dayoffs
 ,pkg_rpt_timetables.timetable_spec_edu(ts.id)          as hours_edu
 from timetables_tbl t, timetables_specs_tbl ts, employees_tbl e, posts_tbl p, timetables_specs_days tsd, timetables_specs_hours_by_spec tsh
 where ts.emp_id = e.id
   and ts.post_id = p.id
   and ts.timetable_id = t.id
   and ts.id = tsd.timetable_spec_id(+)
   and ts.id = tsh.timetable_spec_id(+)
   and tsd.timetable_id(+) = ts.timetable_id
   and tsh.timetable_id(+) = ts.timetable_id
;
